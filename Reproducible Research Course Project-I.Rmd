...
```{r, echo = TRUE}
setwd("E:/R/datasciencecoursera/repdata_data_activity")
activity<-read.csv("activity.csv")
```

Exploring the properties of the dataset
```{r}
dim(activity)
names(activity)
head(activity)
...
par(mfrow=c(1,2))
plot(y=Q6.1$Missing,x=Q6.1$date,main="Missing Value Distribution by Date",width=6)
plot(y=Q6.2$Missing,x=Q6.2$Interval,main="Missing Value Distribution by Interval",width=6)
table(activity$date)
```
